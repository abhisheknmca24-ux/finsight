...
function getMonthlyAverageByType(transactions, type) {
  ...
  (transactions || []).forEach((tx) => {
    ...
    totalsByMonth.set(monthKey, (totalsByMonth.get(monthKey) || 0) + Number(tx.amount || 0));
  });

  const monthlyTotals = Array.from(totalsByMonth.values());
  if (monthlyTotals.length === 0) return 0;

  const avg = monthlyTotals.reduce((sum, value) => sum + value, 0) / monthlyTotals.length;
  return Number(avg.toFixed(2));
}
