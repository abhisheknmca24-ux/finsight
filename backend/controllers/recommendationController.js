...
async function computeRecommendationsForUser(userId) {
  ...
  // If the user has no transactions at all, return a clear no-data response
  ...
  // 1. Extract Current 30-day Window for recent ratios
  const currentTx = [];
  ...
  // Sum recent 30-day window
  currentTx.forEach((t) => {
    if (t.type === "income") income += t.amount;
    else if (t.type === "expense") {
      expense += t.amount;
      categoryExpenses[(t.category || "other").toLowerCase()] = (categoryExpenses[(t.category || "other").toLowerCase()] || 0) + Number(t.amount || 0);
    } else if (t.type === "investment") {
      investment += t.amount;
    }
  });

  // If recent window lacks income/expense data, fall back to monthly averages from history
  const histIncomeAvg = getMonthlyAverageByType(allTransactions, "income");
  const histExpenseAvg = getMonthlyAverageByType(allTransactions, "expense");
  const histInvestmentAvg = getMonthlyAverageByType(allTransactions, "investment");

  if (income === 0 && histIncomeAvg > 0) income = histIncomeAvg;
  if (expense === 0 && histExpenseAvg > 0) {
    expense = histExpenseAvg;

    // compute monthly-average per-category if recent expense is absent
    const { series } = getExpenseSeriesValues(allTransactions);
    const monthsUsed = Math.max(1, series.length);
    const totalsByCategory = {};
    allTransactions.forEach((t) => {
      if (t.type !== "expense") return;
      const cat = (t.category || "other").toLowerCase();
      totalsByCategory[cat] = (totalsByCategory[cat] || 0) + Number(t.amount || 0);
    });
    Object.keys(totalsByCategory).forEach((cat) => {
      categoryExpenses[cat] = totalsByCategory[cat] / monthsUsed;
    });
  }
  if (investment === 0 && histInvestmentAvg > 0) investment = histInvestmentAvg;
  ...
  return {
    ...
    summary: {
      ...
      recentTransactions: currentTx.length,
...
exports.getRecommendations = async (req, res) => {
  try {
    const payload = await computeRecommendationsForUser(req.user.id);
...
// Export helper for dev/test routes
exports.computeRecommendationsForUser = computeRecommendationsForUser;
=======
